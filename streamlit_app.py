...
st.set_page_config(
    page_title="Dalia Pro Autonomous",
    page_icon=ICON_FILENAME,
    layout="wide"
)
...
# 1. Datos, indicadores y backtesting
# --------------------------------------------------
...
def ejecutar_backtesting_5anos(df):
    ...
    df = df.copy()
    df["Signal"] = np.where(
        (df["RSI"] < 35) & (df["Close"] <= df["BB_Lower"] * 1.01),
        1,
        0
    )
...
# 2. Seguridad, sentimiento y riesgo
# --------------------------------------------------
def get_market_sentiment(symbol):
    noticias_ejemplo = (
        f"Mercado para {symbol} experimenta estabilidad, "
        "con flujos institucionales moderados y perspectivas macroeconómicas equilibradas."
    )
...
def circuit_breaker_active():
    try:
        ...
        df_hoy = pd.read_sql_query(
            "SELECT * FROM ordenes WHERE timestamp >= date('now')",
            conn
        )
        conn.close()
...
def registrar_post_mortem(order_id, symbol, resultado, rsi_val, sentiment_val):
    try:
        ...
        cursor.execute(
            ...
            (order_id, symbol, resultado, contexto_rsi, contexto_sentimiento)
            VALUES (?, ?, ?, ?, ?)
            """,
...
# 3. Heartbeat y vigilancia activa
# --------------------------------------------------
...
if "heartbeat_thread" not in st.session_state:
    st.session_state.heartbeat_thread = HeartbeatSystem()
    st.session_state.heartbeat_thread.start()

heartbeat_thread = st.session_state.heartbeat_thread
            ...
            try:
                ...
                for orden in ordenes_abiertas:
                    ...
                    if precio_actual >= tp:
                        ...
                        cursor.execute(
                            "UPDATE ordenes SET tp = ?, sl = ? WHERE id = ?",
                            (nuevo_tp, nuevo_sl, oid)
                        )
                        conn.commit()
...
if "vigilancia_thread" not in st.session_state:
    st.session_state.vigilancia_thread = VigilanciaActivaEngine()
    st.session_state.vigilancia_thread.start()

vigilancia_thread = st.session_state.vigilancia_thread
...
def construir_grafico_avanzado(df, symbol):
    ...
    fig = make_subplots(
        ...
        row_heights=[0.7, 0.3],
        subplot_titles=(
            f"Monitoreo en Tiempo Real — {symbol} (Velas + Bollinger + S&R)",
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="lines",
            name="Soporte IA",
            line=dict(color="#00E676", dash="dot")
        ),
        row=1,
        col=1
    )
...
class DaliaBrokerRouter:
    ...
    def ejecutar_orden(self, symbol, qty, sl, tp, precio_ref):
        if circuit_breaker_active():
            return {
                "status": "BLOCKED",
                "message": "Circuit Breaker activado por exceso de órdenes o riesgo diario."
            }

        sentiment = get_market_sentiment(symbol)

        if sentiment < -0.2:
            registrar_post_mortem(
                str(uuid.uuid4())[:8],
                symbol,
                "BLOQUEADO_SENTIMIENTO",
                50.0,
                sentiment
            )
            return {
                "status": "BLOCKED",
                "message": f"Filtro NLP bloqueado: sentimiento muy negativo ({sentiment:.2f})"
            }
        ...
        c.execute(
            """
            INSERT INTO ordenes 
            (id, symbol, qty, precio, sl, tp, broker, modo, estado)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'ABIERTA')
            """,
...
st.sidebar.title("DALIA PRO AUTONOMOUS")
...
ticker_elegido = st.sidebar.selectbox(
    "🎯 Seleccionar Activo / Ticker",
    options=LISTA_MULTIACTIVO,
    index=0
)
...
modo_produccion_real = st.sidebar.toggle(
    "🚨 CONECTAR A DINERO REAL",
    value=False
)
...
with tab_grafico:
    ...
    with col_s1:
        ...
        st.markdown(
            f"**Score de Sentimiento Narrativo (NLP):** :{color_sent}[{sentiment_score:.2f}]"
        )
    ...
    if df_ai is not None and not df_ai.empty:
        ...
        c3.metric("Soporte Clave", f"${soporte_val:.2f}")
        ...
        if st.button("🧪 Simular Detección de Entrada por la IA"):
            if sentiment_score < -0.2:
                st.warning("⚠️ La IA ignoró la simulación por sentimiento negativo.")
            else:
                conn = sqlite3.connect("dalia_pro_trading.db", timeout=30)
                c = conn.cursor()
                ...
                c.execute(
                    ...
                    (id, symbol, precio, rsi, vwap, volume_score, support, resistance, sl, tp, timestamp, status)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'PENDING')
                    """,
                    (
                        str(uuid.uuid4())[:8],
                ...
                st.success("Señal detectada y enviada a la cola de confirmación.")
                st.rerun()
...
# --------------------------------------------------
with tab_backtest:
    st.subheader(f"🧪 Auditoría de Backtesting Histórico para {ticker_elegido}")
