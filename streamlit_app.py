...
# Conversión dinámica de SVG a PNG de Alta Resolución para accesos directos de Windows/Surface, iOS y Android
components.html(f"""
    <script>
    ...
    function injectPngAppIcons() {{
        ...
        // Renderizar el SVG en un Canvas de 512x512 para generar un PNG real de alta calidad
        const img = new Image();
        ...
        img.onload = function() {{
            ...
            // Icono para accesos directos de Microsoft Edge / Chrome (PWA)
            ...
            if (!shortcutIcon) {{
                ...
                parentHead.appendChild(shortcutIcon);
            }}
            shortcutIcon.href = pngUrl;
        }};
        img.src = svgData;
    }}
    ...
    if (window.parent.document.readyState === 'complete' || window.parent.document.readyState === 'interactive') {{
        ...
        window.parent.addEventListener('DOMContentLoaded', injectPngAppIcons);
    }}
    </script>
""", height=0, width=0)
